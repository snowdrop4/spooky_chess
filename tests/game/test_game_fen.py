...
def test_standard_game_fen_handling() -> None:
    ...
    game = rust_chess.Game(width=8, height=8, fen=test_fen, castling_enabled=True)
...
def test_standard_game_fen_parsing_and_generation_roundtrip() -> None:
    ...
    for fen in test_fens:
        game = rust_chess.Game(width=8, height=8, fen=fen, castling_enabled=True)
    ...
    for input_fen, expected_fen in invalid_ep_fens:
        game = rust_chess.Game(width=8, height=8, fen=input_fen, castling_enabled=True)
        result_fen = game.to_fen()
...
def test_invalid_fen_handling() -> None:
    ...
    for invalid_fen in invalid_fens:
        with pytest.raises(ValueError):  # noqa: PT011
            _game = rust_chess.Game(width=8, height=8, fen=invalid_fen, castling_enabled=True)
