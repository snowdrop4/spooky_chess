use super::*;
use crate::r#move::MoveFlags;
use crate::pieces::PieceType;
use crate::position::Position;
use rand::SeedableRng;
use rand::prelude::IndexedRandom;

type Game8x8 = Game<8, 8>;
type Game10x10 = Game<10, 10>;

#[test]
fn san_basic_pawn_moves() {
    let mut game = Game8x8::standard();
    let mv = game
        .move_from_lan("e2e4")
        .expect("san_basic_pawn_moves: failed to parse e2e4");
    assert_eq!(game.move_to_san(&mv), "e4");

    let mv2 = game
        .move_from_lan("d2d3")
        .expect("san_basic_pawn_moves: failed to parse d2d3");
    assert_eq!(game.move_to_san(&mv2), "d3");
}

#[test]
fn san_knight_move() {
    let mut game = Game8x8::standard();
    let mv = game
        .move_from_lan("g1f3")
        .expect("san_knight_move: failed to parse g1f3");
    assert_eq!(game.move_to_san(&mv), "Nf3");
}

#[test]
fn san_bishop_move() {
    // 1. e4 e5 2. Bc4
    let mut game = Game8x8::standard();
    game.make_move(
        &game
            .move_from_lan("e2e4")
            .expect("san_bishop_move: failed to parse e2e4"),
    );
    game.make_move(
        &game
            .move_from_lan("e7e5")
            .expect("san_bishop_move: failed to parse e7e5"),
    );
    let mv = game
        .move_from_lan("f1c4")
        .expect("san_bishop_move: failed to parse f1c4");
    assert_eq!(game.move_to_san(&mv), "Bc4");
}

#[test]
fn san_capture() {
    // 1. e4 d5 2. exd5
    let mut game = Game8x8::standard();
    game.make_move(
        &game
            .move_from_lan("e2e4")
            .expect("san_capture: failed to parse e2e4"),
    );
    game.make_move(
        &game
            .move_from_lan("d7d5")
            .expect("san_capture: failed to parse d7d5"),
    );
    let mv = game
        .move_from_lan("e4d5")
        .expect("san_capture: failed to parse e4d5");
    assert_eq!(game.move_to_san(&mv), "exd5");
}

#[test]
fn san_piece_capture() {
    // 1. e4 e5 2. Bc4 Nc6 3. Bxf7+
    let mut game = Game8x8::standard();
    game.make_move(
        &game
            .move_from_lan("e2e4")
            .expect("san_piece_capture: failed to parse e2e4"),
    );
    game.make_move(
        &game
            .move_from_lan("e7e5")
            .expect("san_piece_capture: failed to parse e7e5"),
    );
    game.make_move(
        &game
            .move_from_lan("f1c4")
            .expect("san_piece_capture: failed to parse f1c4"),
    );
    game.make_move(
        &game
            .move_from_lan("b8c6")
            .expect("san_piece_capture: failed to parse b8c6"),
    );
    let mv = game
        .move_from_lan("c4f7")
        .expect("san_piece_capture: failed to parse c4f7");
    assert_eq!(game.move_to_san(&mv), "Bxf7+");
}

#[test]
fn san_disambiguation_file() {
    // Two rooks on same rank, different files
    let fen2 = "4k3/8/8/8/8/8/4K3/R6R w - - 0 1";
    let mut game =
        Game8x8::new(fen2, false).expect("san_disambiguation_file: failed to create game from FEN");
    let mv = game
        .move_from_lan("a1d1")
        .expect("san_disambiguation_file: failed to parse a1d1");
    assert_eq!(game.move_to_san(&mv), "Rad1");
    let mv2 = game
        .move_from_lan("h1d1")
        .expect("san_disambiguation_file: failed to parse h1d1");
    assert_eq!(game.move_to_san(&mv2), "Rhd1");
}

#[test]
fn san_disambiguation_rank() {
    // Two rooks on same file, different ranks
    let fen = "4k3/8/8/8/R7/8/8/R3K3 w - - 0 1";
    let mut game =
        Game8x8::new(fen, false).expect("san_disambiguation_rank: failed to create game from FEN");
    let mv = game
        .move_from_lan("a1a2")
        .expect("san_disambiguation_rank: failed to parse a1a2");
    assert_eq!(game.move_to_san(&mv), "R1a2");
    let mv2 = game
        .move_from_lan("a4a2")
        .expect("san_disambiguation_rank: failed to parse a4a2");
    assert_eq!(game.move_to_san(&mv2), "R4a2");
}

#[test]
fn san_multi_digit_destination_rank() {
    let fen = "10/R9/9k/10/10/10/10/10/10/4K5 w - - 0 1";
    let mut game = Game10x10::new(fen, false)
        .expect("san_multi_digit_destination_rank: failed to create game from FEN");
    let mv = game
        .move_from_lan("a9a10")
        .expect("san_multi_digit_destination_rank: failed to parse a9a10");
    assert_eq!(game.move_to_san(&mv), "Ra10");
    let reparsed = game
        .move_from_san("Ra10")
        .expect("san_multi_digit_destination_rank: failed to parse SAN Ra10");
    assert_eq!(reparsed, mv);
}

#[test]
fn san_multi_digit_rank_disambiguation() {
    let fen = "R9/9k/10/10/10/10/10/10/10/R3K5 w - - 0 1";
    let mut game = Game10x10::new(fen, false)
        .expect("san_multi_digit_rank_disambiguation: failed to create game from FEN");
    let mv = game
        .move_from_lan("a10a5")
        .expect("san_multi_digit_rank_disambiguation: failed to parse a10a5");
    assert_eq!(game.move_to_san(&mv), "R10a5");
    let reparsed = game
        .move_from_san("R10a5")
        .expect("san_multi_digit_rank_disambiguation: failed to parse SAN R10a5");
    assert_eq!(reparsed, mv);
}

#[test]
fn san_castling_kingside() {
    let fen = "r3k2r/pppppppp/8/8/8/8/PPPPPPPP/R3K2R w KQkq - 0 1";
    let mut game =
        Game8x8::new(fen, true).expect("san_castling_kingside: failed to create game from FEN");
    let mv = game
        .move_from_lan("e1g1")
        .expect("san_castling_kingside: failed to parse e1g1");
    assert_eq!(game.move_to_san(&mv), "O-O");
}

#[test]
fn san_castling_queenside() {
    let fen = "r3k2r/pppppppp/8/8/8/8/PPPPPPPP/R3K2R w KQkq - 0 1";
    let mut game =
        Game8x8::new(fen, true).expect("san_castling_queenside: failed to create game from FEN");
    let mv = game
        .move_from_lan("e1c1")
        .expect("san_castling_queenside: failed to parse e1c1");
    assert_eq!(game.move_to_san(&mv), "O-O-O");
}

#[test]
fn san_promotion() {
    let fen = "k7/4P3/8/8/8/8/8/4K3 w - - 0 1";
    let mut game = Game8x8::new(fen, false).expect("san_promotion: failed to create game from FEN");
    let mv = game
        .move_from_lan("e7e8q")
        .expect("san_promotion: failed to parse e7e8q");
    assert_eq!(game.move_to_san(&mv), "e8=Q+");
}

#[test]
fn san_promotion_capture() {
    let fen = "3n1k2/4P3/8/8/8/8/8/4K3 w - - 0 1";
    let mut game =
        Game8x8::new(fen, false).expect("san_promotion_capture: failed to create game from FEN");
    let mv = game
        .move_from_lan("e7d8n")
        .expect("san_promotion_capture: failed to parse e7d8n");
    assert_eq!(game.move_to_san(&mv), "exd8=N");
}

#[test]
fn san_checkmate() {
    // Fool's mate: 1. f3 e5 2. g4 Qh4#
    let mut game = Game8x8::standard();
    game.make_move(
        &game
            .move_from_lan("f2f3")
            .expect("san_checkmate: failed to parse f2f3"),
    );
    game.make_move(
        &game
            .move_from_lan("e7e5")
            .expect("san_checkmate: failed to parse e7e5"),
    );
    game.make_move(
        &game
            .move_from_lan("g2g4")
            .expect("san_checkmate: failed to parse g2g4"),
    );
    let mv = game
        .move_from_lan("d8h4")
        .expect("san_checkmate: failed to parse d8h4");
    assert_eq!(game.move_to_san(&mv), "Qh4#");
}

#[test]
fn san_en_passant() {
    let fen = "4k3/8/8/3pP3/8/8/8/4K3 w - d6 0 1";
    let mut game =
        Game8x8::new(fen, false).expect("san_en_passant: failed to create game from FEN");
    let mv = game
        .move_from_lan("e5d6")
        .expect("san_en_passant: failed to parse e5d6");
    assert_eq!(game.move_to_san(&mv), "exd6");
}

#[test]
fn san_from_basic() {
    let mut game = Game8x8::standard();
    let mv = game
        .move_from_san("e4")
        .expect("san_from_basic: failed to parse SAN e4");
    assert_eq!(mv.src, Position::new(4, 1));
    assert_eq!(mv.dst, Position::new(4, 3));
}

#[test]
fn san_from_knight() {
    let mut game = Game8x8::standard();
    let mv = game
        .move_from_san("Nf3")
        .expect("san_from_knight: failed to parse SAN Nf3");
    assert_eq!(mv.src, Position::new(6, 0));
    assert_eq!(mv.dst, Position::new(5, 2));
}

#[test]
fn san_from_castling() {
    let fen = "r3k2r/pppppppp/8/8/8/8/PPPPPPPP/R3K2R w KQkq - 0 1";
    let mut game =
        Game8x8::new(fen, true).expect("san_from_castling: failed to create game from FEN");
    let mv = game
        .move_from_san("O-O")
        .expect("san_from_castling: failed to parse SAN O-O");
    assert!(mv.flags.contains(MoveFlags::CASTLE));
    assert!(mv.dst.col > mv.src.col);

    let mv2 = game
        .move_from_san("O-O-O")
        .expect("san_from_castling: failed to parse SAN O-O-O");
    assert!(mv2.flags.contains(MoveFlags::CASTLE));
    assert!(mv2.dst.col < mv2.src.col);
}

#[test]
fn san_from_castling_zeros() {
    let fen = "r3k2r/pppppppp/8/8/8/8/PPPPPPPP/R3K2R w KQkq - 0 1";
    let mut game =
        Game8x8::new(fen, true).expect("san_from_castling_zeros: failed to create game from FEN");
    let mv = game
        .move_from_san("0-0")
        .expect("san_from_castling_zeros: failed to parse SAN 0-0");
    assert!(mv.flags.contains(MoveFlags::CASTLE));
}

#[test]
fn san_from_promotion() {
    let fen = "k7/4P3/8/8/8/8/8/4K3 w - - 0 1";
    let mut game =
        Game8x8::new(fen, false).expect("san_from_promotion: failed to create game from FEN");
    let mv = game
        .move_from_san("e8=Q")
        .expect("san_from_promotion: failed to parse SAN e8=Q");
    assert_eq!(mv.promotion, Some(PieceType::Queen));
    assert_eq!(mv.dst, Position::new(4, 7));
}

#[test]
fn san_from_with_check_suffix() {
    let mut game = Game8x8::standard();
    // Should ignore the + suffix
    let mv = game
        .move_from_san("Nf3+")
        .expect("san_from_with_check_suffix: failed to parse SAN Nf3+");
    assert_eq!(mv.dst, Position::new(5, 2));
}

#[test]
fn san_from_disambiguation() {
    let fen = "4k3/8/8/8/8/8/4K3/R6R w - - 0 1";
    let mut game =
        Game8x8::new(fen, false).expect("san_from_disambiguation: failed to create game from FEN");
    let mv = game
        .move_from_san("Rad1")
        .expect("san_from_disambiguation: failed to parse SAN Rad1");
    assert_eq!(mv.src, Position::new(0, 0)); // a1
    assert_eq!(mv.dst, Position::new(3, 0)); // d1
}

#[test]
fn san_from_error_invalid() {
    let mut game = Game8x8::standard();
    assert!(game.move_from_san("Zz9").is_err());
    assert!(game.move_from_san("").is_err());
}

#[test]
fn san_roundtrip_all_legal_moves() {
    // From starting position, every legal move should roundtrip through SAN
    let mut game = Game8x8::standard();
    let legal = game.legal_moves();
    for mv in &legal {
        let san = game.move_to_san(mv);
        let parsed = game.move_from_san(&san).unwrap_or_else(|e| {
            panic!(
                "Failed to parse SAN '{}' (from move {}): {}",
                san,
                mv.to_lan(),
                e
            )
        });
        assert_eq!(parsed.src, mv.src, "SAN roundtrip failed for {}", san);
        assert_eq!(parsed.dst, mv.dst, "SAN roundtrip failed for {}", san);
        assert_eq!(
            parsed.promotion, mv.promotion,
            "SAN roundtrip failed for {}",
            san
        );
    }
}

#[test]
fn san_roundtrip_midgame() {
    // Test roundtrip from a more complex position with captures, checks possible
    let fen = "r1bqkb1r/pppp1ppp/2n2n2/4p3/2B1P3/5N2/PPPP1PPP/RNBQK2R w KQkq - 4 4";
    let mut game =
        Game8x8::new(fen, true).expect("san_roundtrip_midgame: failed to create game from FEN");
    let legal = game.legal_moves();
    for mv in &legal {
        let san = game.move_to_san(mv);
        let parsed = game.move_from_san(&san).unwrap_or_else(|e| {
            panic!(
                "Failed to parse SAN '{}' (from {}): {}",
                san,
                mv.to_lan(),
                e
            )
        });
        assert_eq!(parsed.src, mv.src, "Roundtrip src failed for {}", san);
        assert_eq!(parsed.dst, mv.dst, "Roundtrip dst failed for {}", san);
    }
}

#[test]
fn san_roundtrip_random_games() {
    let mut rng = rand::rngs::SmallRng::seed_from_u64(123);

    for _game_num in 0..100 {
        let mut game = Game8x8::standard();
        for _move_num in 0..100 {
            if game.is_over() {
                break;
            }
            let legal = game.legal_moves();
            if legal.is_empty() {
                break;
            }
            // Test roundtrip for all legal moves
            for mv in &legal {
                let san = game.move_to_san(mv);
                let parsed = game.move_from_san(&san).unwrap_or_else(|e| {
                    panic!(
                        "Game {}, move {}: Failed to parse SAN '{}' (from {}): {}",
                        _game_num,
                        _move_num,
                        san,
                        mv.to_lan(),
                        e
                    )
                });
                assert_eq!(parsed.src, mv.src);
                assert_eq!(parsed.dst, mv.dst);
                assert_eq!(parsed.promotion, mv.promotion);
            }
            let chosen = legal
                .choose(&mut rng)
                .expect("san_roundtrip_random_games: legal moves list must not be empty");
            game.make_move_unchecked(chosen);
        }
    }
}
