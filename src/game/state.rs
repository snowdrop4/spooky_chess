    ...
    [(); (W * H).div_ceil(64)]:,
{
    ...
    fn has_any_legal_move(&mut self) -> bool {
        ...
        if self.castling_enabled && W >= 5 && info.num_checkers == 0 {
            if self.castling_rights.has_kingside(color) {
                ...
                if king_dst < rook_col
                    && self
                        .try_castle_pin_aware(&king_pos, king_pos.row, king_dst, rook_col, &info)
                        .is_some()
                {
                    return true;
                }
            ...
            if self.castling_rights.has_queenside(color) && king_pos.col >= 2 {
                let king_dst = king_pos.col - 2;
                if self
                    .try_castle_pin_aware(&king_pos, king_pos.row, king_dst, 0, &info)
                    .is_some()
                {
                    return true;
                }
        ...
        // Double check: only king moves are legal (already checked above)
        ...
        for idx in self.board.color_bb(color).iter_ones() {
            ...
            match pt {
                PieceType::Knight => {
                    if !(geo.knight_attacks(idx).andnot(own) & move_mask).is_empty() {
                        return true;
                    }
                }
                PieceType::Bishop => {
                    if !(geo.diagonal_attacks(idx, occupied).andnot(own) & move_mask).is_empty() {
                        return true;
                    }
                }
                PieceType::Rook => {
                    if !(geo.orthogonal_attacks(idx, occupied).andnot(own) & move_mask).is_empty() {
                        return true;
                    }
                }
                PieceType::Queen => {
                    ...
                    if !(attacks.andnot(own) & move_mask).is_empty() {
                        return true;
                    }
                }
                PieceType::Pawn => {
                    ...
                    if pos.row == start_row && !push.is_empty() {
                        ...
                        if !(double & move_mask).is_empty() {
                            return true;
                        }
                    }

                    // Captures
                    let pawn_attacks = geo.pawn_attacks(idx, is_white);
                    if !(pawn_attacks & enemy & move_mask).is_empty() {
                        return true;
                    }

                    // En passant (make/unmake fallback)
                    if let Some(ep) = self.en_passant {
                        let ep_bb = Bitboard::single(ep.to_index(W));
                        if !(pawn_attacks & ep_bb).is_empty() {
